package com.next.aap.task.member;
...
@Component
public class ReceiveMemberListener extends AwsQueueListener{

	@Autowired
	public ReceiveMemberListener(@Value("${member_queue_name}") String queueName,@Value("${aws_access_key}") String accessKey
			, @Value("${aws_access_secret}") String accessSecret) {
		super(queueName, accessKey, accessSecret, false);
	}
	@Autowired
